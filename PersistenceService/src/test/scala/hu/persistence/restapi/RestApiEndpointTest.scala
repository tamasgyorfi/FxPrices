...
class RestApiEndpointTest extends FunSuite with RestApiRoute with Specs2RouteTest with HttpService with FongoFixture with TestData with BeforeAndAfterAll {
  ...
  test("RestApiEndpoint should return the history of a currency pair with no error") {
    ...
    Get("/currencyPairHistory?ccy1=USD&ccy2=KRW&date=2016-01-11") ~> apiRoute ~> check {
  ...
  test("RestApiEndpoint should return empty list of a currency pair with no error when there is no data on a date") {
    ...
    Get("/currencyPairHistory?ccy1=USD&ccy2=KRW&date=2001-01-11") ~> apiRoute ~> check {
      val response = responseAs[String]
  ...
  test("RestApiEndpoint should return empty list of a currency pair with no error when the currency pair is unknown - ccy1") {
    ...
    Get("/currencyPairHistory?ccy1=USDDD&ccy2=KRW&date=2016-01-11") ~> apiRoute ~> check {
      val response = responseAs[String]
      assert(response === """{"quotes":[],"errorMessage":""}""")
    }
  }
  ...
  test("RestApiEndpoint should return empty list of a currency pair with no error when the currency pair is unknown - ccy2") {
    ...
    Get("/currencyPairHistory?ccy1=USD&ccy2=KPW&date=2016-01-11") ~> apiRoute ~> check {
      val response = responseAs[String]
      assert(response === """{"quotes":[],"errorMessage":""}""")
    }
  }
  ...
  test("RestApiEndpoint should return an error message when the date is not parsable") {
    ...
    Get("/currencyPairHistory?ccy1=USD&ccy2=KRW&date=2001-01-111") ~> apiRoute ~> check {
      val response = responseAs[String]
      assert(response === """{"quotes":[],"errorMessage":"Error: Unable to parse date 2001-01-111"}""")
    }
  }
