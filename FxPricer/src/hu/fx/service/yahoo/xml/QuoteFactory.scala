package hu.fx.service.yahoo.xml
...
class QuoteFactory extends YahooPriceSource {
  ...
  def fromNode(quote: Node): Quote = {
    def createFxQuote(node: Node, currencyName: String): Quote = {
      FxQuote(
    ...
    def createPmQuote(node: Node, currencyName: String): Quote = {
      PmQuote(
        currencyName,
        getAttribute(node, VOLUME_ATTRIBUTE) toInt,
        getAttribute(node, PRICE_ATTRIBUTE) toDouble,
    ...
    if (name.isEmpty) {
      EmptyQuote
    } else if (name.matches(FX_PATTERN)) {
      createFxQuote(quote, name)
    } else {
      createPmQuote(quote, name)
    }
  }
}
